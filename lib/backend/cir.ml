...
let from_ast_struct structure =
  ...
  in
  (* TODO known bug:
   * lhs names in bindings for primops and native functions are not renamed
   * (we'll have to create dummy bindings in AST or cache names in Var_namer --
   * neither is ideal).
   * The correctness of renaming (and unique name generation) depends on these
   * built-in names being disjoint from the names generated by Var_namer. I
   * don't like this implicit dependency, but don't know how to eliminate it
