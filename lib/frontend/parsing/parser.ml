...
and _parse_primary_typ (s : _tok_stream) : Ast.typ = (* int, 'a, _, (a -> b) *)
                        (* NOTE stay synched! *)
  ...
  | Underscore ->
    { Ast.typ_desc = Typ_var None; typ_span = tok.token_span }
  | QuoteIdent name ->
    { Ast.typ_desc = Typ_var (Some name); typ_span = tok.token_span }
  | DecapIdent name ->
